...
mdm.processConfig = function (unprocessedConfig, configFilename) {
  try {
    ...
    processedConfig.options = _(unprocessedConfig.options).pick('meteorite', 'insecure', 'meteorRelease', 'deploysToKeep');
...
mdm.ssh.on('ready', function () {
  ...
  commands.push({ cmd: 'echo "Removing old files..." && rm -rf bundle/ bundle.tgz', output: 'Cleaning: ', quiet: false });
  ...
  commands.push({ cmd: 'echo "Now extracting bundle.tgz..." && tar xf bundle.tgz', output: 'Extracting: ', quiet: false });
