...
mdm.startDeploy = function () {
  ...
  commands.push({ cmd: 'echo "Removing old builds..." && cd builds && rm -rf `ls -r --ignore=current | tail -n +' + (parseInt(mdm.processedConfig.options.buildsToKeep, 10) + 1) + '`', output: 'Cleaning: ', quiet: false });
  ...
  commands.push({ cmd: 'echo "Restarting Node.js via Upstart..." && sudo restart --quiet ' + mdm.currentEnvironment.taskName, output: 'Restarting: ', quiet: false });
