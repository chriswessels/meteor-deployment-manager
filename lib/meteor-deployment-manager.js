...
mdm.startDeploy = function () {
  ...
  commands.push({ cmd: 'echo "Removing old builds..." && cd builds && rm -rf `ls -t | tail -n +' + (parseInt(mdm.processedConfig.options.buildsToKeep, 10) + 1) + '`', output: 'Cleaning: ', quiet: false });
  ...
  commands.push({ cmd: 'echo "Resetting symbolic link for latest build." && cd builds && rm -f current && ln -s ' + timestamp + ' current' });
