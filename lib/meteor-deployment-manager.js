...
mdm.processConfig = function (unprocessedConfig, configFilename) {
  try {
    ...
    processedConfig.options = _(unprocessedConfig.options).pick('meteorite', 'insecure', 'meteorRelease', 'buildsToKeep', 'avoidAutomaticRestart');
...
mdm.startDeploy = function () {
  ...
  if (!mdm.processedConfig.options.avoidAutomaticRestart) {
    ...
    commands.push({ cmd: 'echo "Avoided automatic restart..."', output: 'Keeping current service state', quiet: false });
  }
