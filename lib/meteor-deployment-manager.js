...
mdm.processConfig = function (unprocessedConfig, configFilename) {
  try {
    ...
    _(unprocessedConfig.environments).map(function (value, index, collection) {
      ...
      if (!value.password) { throw { message: "The '" + index + "' environment does not include a password." }; }
  ...
  mdm.ssh.exec(cmdToExecute, function (error, stream) {
    ...
    stream.on('exit', function (code, signal) {
      ...
      if (commands[nextIndex]){
        ...
        logger.info('No further commands to be run.');
        mdm.exitProcess();
      }
    });
    stream.on('data', function (data, extended) {
      if (extended === 'stderr') {
        mdm.ssh.end();
        mdm.die('An error occured while attempting to execute ' + value.cmd + ' on ' + mdm.currentEnvironment.hostname, data.toString().replace(/(^\s*)|(\s*$)/g,''));
      } else if (VERBOSE && value.quiet || !value.quiet) {
...
mdm.ssh.on('ready', function() {
  ...
  commands.push({ cmd: 'cd source && git fetch origin && git checkout --quiet ' + mdm.currentEnvironment.gitBranch + ' && git pull --quiet origin ' + mdm.currentEnvironment.gitBranch, output: 'Updating codebase: ', quiet: false });
  commands.push({ cmd: 'cd source && mrt bundle ../bundle.tgz', output: 'Bundling Application: ', quiet: false });
  commands.push({ cmd: 'tar xf bundle.tgz', output: 'Extracting bundle: ', quiet: false });
  commands.push({ cmd: 'sudo stop app_name && sudo start app_name', output: 'Restarting application:', quiet: false });

  mdm.runCommandLoop(commands, 0);1
});
