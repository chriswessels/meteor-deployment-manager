...
mdm.runCommandLoop = function (commands, index) {
  ...
  try {
    mdm.ssh.exec(value.cmd, function (error, stream) {
      if (error) { throw { message: error }; }
      stream.on('end', function () {
        if (VERBOSE) { logger.info(value.cmd + " ended."); }
      });
      stream.on('close', function () {
        if (VERBOSE) { logger.info(value.cmd + " closed."); }
      });
      ...
      stream.on('data', function (data, extended) {
        if (extended === 'stderr') { throw { message: data }; }
    ...
    logger.error('An error occured while attempting to execute ' + value.cmd + ' on ' + mdm.currentEnvironment.hostname);
    logger.error('Message:', exception.message);
  }
}
...
mdm.ssh.on('ready', function() {
  ...
  var commands = [
    { cmd: 'which node', output: 'Path to Node.js: ', quiet: true },
    { cmd: 'which npm', output: 'Path to NPM: ', quiet: true },
    { cmd: 'node --version', output: 'Node.js Version: ', quiet: false },
    { cmd: 'npm --version', output: 'NPM Version: ', quiet: false }
  ];
